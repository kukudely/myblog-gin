...
func GetArt(pageSize int, pageNum int) ([]Article, int, int64) {
	...
	err = db.Select("articles.id, title, img, `desc`, created_at").Limit(pageSize).Offset((pageNum - 1) * pageSize).Order("Created_At DESC").Joins("Category").Find(&articleList).Error
	// 单独计数
